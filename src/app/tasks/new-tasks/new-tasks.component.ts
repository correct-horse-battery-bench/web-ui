...
export class NewTasksComponent implements OnInit {
  ...
  protected async onSubmit(): Promise<void> {
    if (this.form.valid) {
      ...
      try {
        await firstValueFrom(this.gs.create(SERV.TASKS, this.form.value));
        this.alert.showSuccessMessage('New Task created');
        const success = await this.router.navigate(['tasks/show-tasks']);
        if (!success) {
          console.error('Navigation failed.');
        }
      } catch (error) {
        console.error('Error creating task:', error);
        this.alert.showErrorMessage('Failed to create task. Please try again.');
      } finally {
